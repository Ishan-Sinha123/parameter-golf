...
# Matches SOTA-comparable settings (11L/512d/MLP3x) as baseline.
# Individual experiments override these via env vars.
...
run_one() {
    ...
    (
        ...
        export RUN_ID="$name"
        # 1200s timeout = 20 min max (90s training + up to 18 min TTT eval)
...
run_phase1() {
    ...
    echo "║  (11L/512d/MLP3x, 8×H100, 90s training)      ║"
    ...
    run_one "p1_swiglu" "train_alpha.py" \
        "TTT_MODE=none" "MLP_ACTIVATION=swiglu"

    # 1C. Trigram hash embedding
    run_one "p1_trigram_hash" "train_alpha.py" \
        "TTT_MODE=none" "HASH_MODE=trigram"

    # 1D. SwiGLU + Trigram (both features together)
    run_one "p1_swiglu_trigram" "train_alpha.py" \
        "TTT_MODE=none" "MLP_ACTIVATION=swiglu" "HASH_MODE=trigram"

    # 1E. LeakyReLU² (free activation swap)
    run_one "p1_leaky_relu2" "train_alpha.py" \
...
run_phase3() {
    ...
    run_one "p3_9L_mlp4x_swiglu_trigram" "train_alpha.py" \
        "TTT_MODE=none" "NUM_LAYERS=9" "MLP_MULT=4" \
        "MLP_ACTIVATION=swiglu" "HASH_MODE=trigram"

    # 3B. 8L/MLP4x + SwiGLU + Trigram
    run_one "p3_8L_mlp4x_swiglu_trigram" "train_alpha.py" \
        "TTT_MODE=none" "NUM_LAYERS=8" "MLP_MULT=4" \
        "MLP_ACTIVATION=swiglu" "HASH_MODE=trigram"

    # 3C. 11L (SOTA depth) + SwiGLU + Trigram (safe combo)
    run_one "p3_11L_swiglu_trigram" "train_alpha.py" \
        "TTT_MODE=none" "NUM_LAYERS=11" "MLP_MULT=3" \
        "MLP_ACTIVATION=swiglu" "HASH_MODE=trigram"

    # 3D. Best arch + LeakyReLU² + Trigram (fallback if SwiGLU fails)
    run_one "p3_9L_mlp4x_leaky_trigram" "train_alpha.py" \
        "TTT_MODE=none" "NUM_LAYERS=9" "MLP_MULT=4" \
        "MLP_ACTIVATION=leaky_relu2" "HASH_MODE=trigram"
}
...
echo "║  8×H100 | 90s training | train_alpha.py       ║"
echo "║  Phases: ${PHASES[*]}                              ║"
echo "║  Logs: $LOG_DIR/"
echo "╚═══════════════════════════════════════════════╝"
echo ""
