...
def _sdpa_attn(q, k, v, causal=True):
    """Fallback: PyTorch SDPA. q (B,T,H,D), k/v (B,T,Hkv,D) -> (B,T,H,D)."""
    q = q.transpose(1, 2)
    k = k.transpose(1, 2)
    v = v.transpose(1, 2)
...
class Hyperparameters:
    ...
    ttt_fft_lr = float(os.environ.get("TTT_FFT_LR", 1e-4))
    ttt_lora_targets = os.environ.get("TTT_LORA_TARGETS", "qv")  # qv, qvk, qv_mlp, qvk_mlp
    ttt_lora_decay = float(os.environ.get("TTT_LORA_DECAY", 1.0))
    # --- MLP activation ---
    mlp_activation = os.environ.get("MLP_ACTIVATION", "leaky_relu2")  # leaky_relu2, swiglu
    # Skip post-training quantization (for fast arch experiments)
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int, activation: str = "leaky_relu2"):
        ...
        if activation == "swiglu":
            # SwiGLU needs a gate projection; store as non-banked param
            mlp_dim = int(mlp_mult * dim)
            self.gate_w = nn.Parameter(torch.empty(mlp_dim, dim))
            nn.init.orthogonal_(self.gate_w)
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        if self.activation == "swiglu":
            up = F.linear(x, up_w.to(x.dtype))
            gate = F.linear(x, self.gate_w.to(x.dtype))
            x = F.silu(gate) * up
...
class GPT(nn.Module):
    ...
    def forward_ttt(self, input_ids: Tensor, target_ids: Tensor, lora=None, per_token: bool = False) -> Tensor:
        """Forward pass with optional LoRA adapters for TTT.
        When lora is provided or per_token=True, returns per-token losses (bsz, seq_len).
        ...
        for i in range(self.num_encoder_layers):
            ve = self._get_ve(i, input_ids, ve_cache)
            # Apply LoRA deltas to bank weights if available
            q_w = self.qo_bank[i]
            k_w = self.kv_bank[i]
            ...
            x, raw_v = self.blocks[i](x, x0, q_w, k_w, v_w, o_w, up_w, down_w, v_embed=ve, v0=v0)
            # Apply LoRA on the output if available
            if lora is not None:
                # LoRA is applied as additive correction to the residual stream
                lora_delta = lora.q_loras[i](self.blocks[i].attn_norm(x) * self.blocks[i].ln_scale_factor)
                x = x + lora_delta
            if v0 is None and raw_v is not None:
                v0 = raw_v
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                ...
                v_embed=ve, v0=v0)
            if lora is not None:
                lora_delta = lora.q_loras[bi](self.blocks[bi].attn_norm(x) * self.blocks[bi].ln_scale_factor)
                x = x + lora_delta
        x = self.final_norm(x)
        if self.tie_embeddings:
        ...
        if lora is not None or per_token:
...
class BatchedTTTLoRA(nn.Module):
    """LoRA adapters for banked GPT model. Applies to residual stream via Q projection dimensions."""
    def __init__(self, bsz: int, model: GPT, rank: int, targets: str = "qv"):
        ...
        for i in range(model.num_layers):
            ...
            self.v_loras.append(BatchedLinearLoRA(bsz, dim, model.kv_dim, rank))
            if self.k_loras is not None:
                self.k_loras.append(BatchedLinearLoRA(bsz, dim, model.kv_dim, rank))
            if self.fc_loras is not None:
                self.fc_loras.append(BatchedLinearLoRA(bsz, dim, model.mlp_dim, rank))
                self.proj_loras.append(BatchedLinearLoRA(bsz, model.mlp_dim, dim, rank))
...
def eval_val_ttt_lora(
    ...
    """Evaluate with batched LoRA TTT on the banked SOTA model."""
        ...
        for ci in range(max_nc):
            ...
            if needs_train:
                ...
                for _extra in range(args.ttt_steps - 1):
                    with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                        ptl2 = base_model.forward_ttt(x, y, lora=cur_lora)
...
def eval_val_fft(
    ...
    """FFT eval: fine-tune last N layers per document."""
    files = sorted(glob.glob(args.val_files))
    all_tokens = torch.cat([load_data_shard(Path(f)) for f in files])
    docs = _find_docs(all_tokens)
    rank_docs = docs[(len(docs) * rank) // world_size : (len(docs) * (rank + 1)) // world_size]
    ...
    for bi in range(0, len(rank_docs), batch_size):
        ...
        for ci in range(max_nc):
            ...
            if needs_train:
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    ptl = base_model.forward_ttt(x, y, per_token=True)
            else:
                with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    ptl = base_model.forward_ttt(x, y, per_token=True)
            with torch.no_grad():
...
def quantize_int6_gptq(weight, hessian=None, clip_range=31, block_size=128):
    ...
    try:
        ...
        # Hessian not positive-definite (undertrained model) — fall back to percentile
        return _quantize_int6_percentile(t32, clip_range)
...
class _HessianMLP(nn.Module):
    ...
    def __init__(self, dim, mlp_mult, activation="leaky_relu2"):
        ...
        if activation == "swiglu":
            self.gate = CastedLinear(dim, int(mlp_mult * dim), bias=False)
    def forward(self, x):
        if self.activation == "swiglu":
...
def main() -> None:
    ...
    try:
        ...
        log0("nvidia-smi: not found in PATH", console=False)
    log0("=" * 100, console=False)
    ...
    # SwiGLU gate weights (if using swiglu activation)
    for block in base_model.blocks:
        if hasattr(block.mlp, 'gate_w'):
            scalar_params.append(block.mlp.gate_w)
    ...
    if args.skip_quantize:
        log0("SKIP_QUANTIZE=1: skipping GPTQ/LZMA/sliding-window, using EMA val_bpb as final score")
        ttt_mode = args.ttt_mode
        if ttt_mode != "none":
            log0(f"ttt:starting mode={ttt_mode}")
            torch.cuda.synchronize()
            t_ttt = time.perf_counter()
            if ttt_mode == "lora" or ttt_mode == "bias":
                ttt_loss, ttt_bpb = eval_val_ttt_lora(
                    args, base_model, rank, world_size, device,
                    base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
                )
            elif ttt_mode.startswith("fft"):
                n_fft = int(ttt_mode.replace("fft", "").replace("_all", str(args.num_layers))) if ttt_mode != "fft_all" else args.num_layers
                ttt_loss, ttt_bpb = eval_val_fft(
                    args, base_model, rank, world_size, device,
                    base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
                    num_fft_layers=n_fft,
                )
            else:
                log0(f"ttt:unknown mode {ttt_mode}, skipping")
                ttt_loss, ttt_bpb = 0.0, 0.0
            torch.cuda.synchronize()
            log0(
                f"ttt_eval val_loss:{ttt_loss:.4f} val_bpb:{ttt_bpb:.4f} "
                f"mode:{ttt_mode} eval_time:{1000.0 * (time.perf_counter() - t_ttt):.0f}ms"
            )
