...
Muon momentum of 0.96 may be optimal at the current 8-minute wallclock.
This run is one point of a fine sweep probing just above the default
0.95: moderate momentum balances gradient-history retention against
responsiveness, and when total step count is capped by wallclock a small
shift can meaningfully alter the effective optimization trajectory.
...
- **Seed:** 1337
- **Branch / commit:** `autoresearch-deploy` @ `d7a7125`
...
SOTA-vs-default structural delta, not the effect of momentum itself.
The momentum sweep only makes sense compared across its own cohort
(`exp001`–`exp004`).

## Verdict

**neutral.** 0.96 passes the screening gate with a near-zero quant gap
and the int8+zlib artifact comfortably under the 16 MB cap, and the log
shows a clean monotonic loss descent. But the run is +0.184 nats above
the SOTA baseline because it is not stacked on the SOTA recipe, and
ranking the momentum values requires the sibling sweep points — this
result alone does not support win/regression.
...
- Probe interaction with `MUON_WD`, Parallel Muon, and `warmdown3500`:
  recent SOTA chains changed the effective update scale, which can
