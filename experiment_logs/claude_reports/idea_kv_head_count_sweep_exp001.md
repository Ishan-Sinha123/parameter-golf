...
Reducing GQA key/value head count to `NUM_KV_HEADS=2` (with the default 8 query heads, i.e. 8:2 GQA) should balance KV-projection parameter cost against BPB — shrinking K/V projections without meaningfully hurting loss. Any bytes freed can later be redeployed into depth or MLP width in a stacked recipe.
...
- Seed: `1337`, `world_size:1`, `grad_accum_steps:8`, `train_seq_len:1024`, `train_batch_tokens:524288`.
...
Caveat: the run hit the **540 s screening wallclock cap at step 1680 / 20000** (`stopping_early: wallclock_cap`, ≈8.4% of planned steps) — these are early-training values, not a converged result. No warnings or NaNs emitted.

Baseline val_bpb for delta calc: **1.081**

| Metric | Value | Δ vs baseline (1.081) |
| --- | --- | --- |
| screen EMA val_bpb | 1.30301978 | **+0.22202** |
| gate int6 val_bpb | 1.31600000 | **+0.23500** |
