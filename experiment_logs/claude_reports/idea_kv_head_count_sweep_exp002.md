...
Relevant knobs captured from `train.log`:
...
- **Instrumentation concern.** The tracker recorded `gate_artifact_mb=0.0` and `gate_passed=true` even though the submission exceeds 16 MB decimal. Downstream gates should not trust that signal on this run.
...
- **Pair high-KV ablations with partial RoPE / XSA** so the extra KV capacity has non-trivial routing to do, rather than paying for it in vanilla attention.
